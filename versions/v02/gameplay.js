...
function submitScore(name, triesRemaining) {
    ...
    xmlhttp.send(`action=addScore&name=${name}&triesRemaining=${triesRemaining}`);
}
...
function updateLeaderboard() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.status === 200 && xmlhttp.readyState === 4) {
            const leaderboard = JSON.parse(xmlhttp.responseText);
            displayLeaderboard(leaderboard);
        }
    };
    xmlhttp.send("action=getScores");
}

// Clear the leaderboard for the session
function clearLeaderboard() {
    ...
    xmlhttp.send("action=clearScores");
}

// Display the leaderboard on the page
function displayLeaderboard(leaderboard) {
    const leaderboardCont = document.getElementById("leaderboard");
    leaderboardCont.innerHTML = '';

    leaderboard.forEach((entry, index) => {
        const entryCont = document.createElement("div");
        entryCont.className = "leaderboard-entry";
        entryCont.innerHTML = `<span class="leaderboard-rank">${index + 1}.</span> ${entry.name} - ${entry.triesRemaining} tries remaining`;
        leaderboardCont.appendChild(entryCont);
    });
}
...
function addLetter(key) {
    ...
    if (!gameEnd && currRow == TOTAL_TRIES) {
        ...
        // Prompt user for name and submit score
        setTimeout(() => {
            // Prompt user for name and submit score
            popupWithInput("Game over! Enter your name:", (playerName) => {
                if (playerName) {
                    submitScore(playerName, TOTAL_TRIES - currRow);
                }
            });
        }, 2500);
    }
...
function checkWord() {
    ...
    if (correctPositions.length === WORD_LEN) {
        ...
        setTimeout(() => {
            // Prompt user for name and submit score
            popupWithInput("Impressive! Enter your name:", (playerName) => {
                if (playerName) {
                    submitScore(playerName, TOTAL_TRIES - currRow);
                }
            });
...
// Popup with input field for user to enter their name
function popupWithInput(message, callback) {
    const popup = document.getElementById("popup");
    popup.innerHTML = `
...
window.onload = function() {
    ...
    // Event listener for reseting leaderboard
